...
#define CUDA_ERROR(result) MessageBoxA(0, cu::Result(result), "Error", MB_OK); 
...
static AddInX xai_cuda(
	ArgsX(CATEGORY)
	.Documentation(_T("CUDA driver API functions."))
);
...
XLL_ENUM(CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_LAYERED_WIDTH, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_LAYERED_WIDTH, CATEGORY, _T("Maximum 2D layered texture width."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_LAYERED_HEIGHT, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_LAYERED_HEIGHT, CATEGORY, _T("Maximum 2D layered texture height."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_LAYERED_LAYERS, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE2D_LAYERED_LAYERS, CATEGORY, _T("Maximum layers in a 2D layered texture."));
...
XLL_ENUM(CU_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE, CU_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE, CATEGORY, _T("Peak memory clock frequency in kilohertz."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_GLOBAL_MEMORY_BUS_WIDTH, CU_DEVICE_ATTRIBUTE_GLOBAL_MEMORY_BUS_WIDTH, CATEGORY, _T("Global memory bus width in bits."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_L2_CACHE_SIZE, CU_DEVICE_ATTRIBUTE_L2_CACHE_SIZE, CATEGORY, _T("Size of L2 cache in bytes."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR, CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR, CATEGORY, _T("Maximum resident threads per multiprocessor."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_ASYNC_ENGINE_COUNT, CU_DEVICE_ATTRIBUTE_ASYNC_ENGINE_COUNT, CATEGORY, _T("Number of asynchronous engines."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_UNIFIED_ADDRESSING, CU_DEVICE_ATTRIBUTE_UNIFIED_ADDRESSING, CATEGORY, _T("Device uses shares a unified address space with the host."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE1D_LAYERED_WIDTH, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE1D_LAYERED_WIDTH, CATEGORY, _T("Maximum 1D layered texture width."));
XLL_ENUM(CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE1D_LAYERED_LAYERS, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE1D_LAYERED_LAYERS, CATEGORY, _T("Maximum layers in a 1D layered texture."));
...
XLL_ENUM(CU_CTX_SCHED_BLOCKING_SYNC, CU_CTX_SCHED_BLOCKING_SYNC, CATEGORY, 
	_T("Set blocking synchronization as default scheduling. "));
...
static AddInX xai_cuCtx(
	...
	.Arg(XLL_WORDX, _T("Flags"), _T("Create an event. ")
								 _T("using flags from the CU_CTX_SCHED_* enumeration. ")) 
...
static AddInX xai_cuStreamSynchronize(
	...
	.Arg(XLL_HANDLEX, _T("End"), _T("is a handle to the ending event. ")) 
	.Category(CATEGORY)
...
#pragma XLLEXPORT
	try {
		ensure (CUDA_SUCCESS == cuStreamSynchronize(*handle<cu::Stream> (stream)));
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
...
double WINAPI
xll_cuEventElapsedTime(HANDLEX start, HANDLEX end)
{
#pragma XLLEXPORT
	float ms(std::numeric_limits<float>::quiet_NaN());

	try {
		ensure (CUDA_SUCCESS == cuEventElapsedTime(&ms, *handle<cu::Event>(start), *handle<cu::Event>(end)));
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());

		ms = std::numeric_limits<float>::quiet_NaN();
	}
...
#pragma XLLEXPORT
	try {
		ensure (CUDA_SUCCESS == cuEventRecord(*handle<cu::Event>(event), *handle<cu::Stream>(stream)));
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
...
#pragma XLLEXPORT
	try {
		ensure (CUDA_SUCCESS == cuEventSynchronize(*handle<cu::Event>(event)));
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
...
#pragma XLLEXPORT
	...
	try{
		h = new cu::Module(cu::Module::Load(name));
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
	}
...
#pragma XLLEXPORT
	...
	try{
		f = handle<cu::Module>(module)->GetFunction(name);
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
	}
